...
export function WhiteboardCanvas({ batches, onWarning }: WhiteboardCanvasProps) {
  ...
  return (
    <section className="relative h-full overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-[var(--color-paper)] shadow-[var(--shadow-card)]">
      ...
      <div data-testid="whiteboard-stats" className="glass-panel pointer-events-none absolute left-3 top-3 rounded-xl px-3 py-2 text-xs text-[var(--color-text-secondary)] shadow-[var(--shadow-card)]">
        <div data-testid="whiteboard-zoom">Zoom: {(camera.zoom * 100).toFixed(0)}%</div>
        <div data-testid="whiteboard-committed">Committed: {stats.committed}</div>
        <div data-testid="whiteboard-active">Active: {stats.active}</div>
      </div>
    </section>
  );
}
